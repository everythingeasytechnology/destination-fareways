...
@section('content')
<div class="mb-4">
    <h2 class="display-font mb-1 text-navy">Edit FAQ</h2>
    <p class="text-muted mb-0">Modify question formulations, adjust answers, and target different page paths.</p>
</div>
                ...
                <div class="row g-3">
                    <div class="col-12">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control px-3" id="question" name="question" value="{{ old('question', $faq->question) }}" placeholder="e.g. Can I change my booking after purchase?" required>
                        </div>
                    </div>
                    ...
                    <div class="col-12">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control tinymce-editor" id="answer" name="answer" rows="8">{{ old('answer', $faq->answer) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card premium-card border-0 shadow-sm p-4">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select px-3 font-monospace" id="page_slug" name="page_slug" required style="font-size: 0.85rem;">
                        <option value="global" {{ old('page_slug', $faq->page_slug) === 'global' ? 'selected' : '' }}>🌎 Global (All Pages)</option>
                        @foreach($pages as $page)
                            <option value="{{ $page->slug }}" {{ old('page_slug', $faq->page_slug) === $page->slug ? 'selected' : '' }}>/{{ $page->slug }} ({{ $page->title }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control px-3" id="category" name="category" value="{{ old('category', $faq->category) }}" placeholder="e.g. Bookings, Payments">
                </div>

                <div class="mb-3">
                    <label for="sort_order" class="form-label fw-bold">Sort Priority Order</label>
                    <input type="number" class="form-control px-3" id="sort_order" name="sort_order" value="{{ old('sort_order', $faq->sort_order) }}" min="0" required>
                </div>

                <div class="mb-3">
                    <label for="status" class="form-label fw-bold">Status</label>
                    <select class="form-select px-3" id="status" name="status" required>
                        <option value="active" {{ $faq->status === 'active' ? 'selected' : '' }}>Active (Visible)</option>
                        <option value="inactive" {{ $faq->status === 'inactive' ? 'selected' : '' }}>Inactive (Draft)</option>
                    </select>
                </div>
            </div>
        </div>
