...
@section('title', 'Add Structured Schema')
...
<div class="mb-4">
    ...
    <h2 class="display-font mb-1 text-navy">Add Structured Schema Markup</h2>
    <p class="text-muted mb-0">Build rich data schemas, select structural categories, and validate the payload structure in real time.</p>
</div>

<form action="{{ route('admin.schema.store') }}" method="POST" id="schema-form">
    @csrf
        ...
        <div class="col-lg-7">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('page_identifier') is-invalid @enderror" id="page_identifier_select" required>
                            ...
                            @foreach($pages as $key => $name)
                                <option value="{{ $key }}" {{ old('page_identifier') == $key ? 'selected' : '' }}>{{ $name }} ({{ $key }})</option>
                            @endforeach
                            <option value="custom" {{ old('page_identifier') && !array_key_exists(old('page_identifier'), $pages) ? 'selected' : '' }}>[Other... Create Custom Identifier]</option>
                        </select>
                        <input type="hidden" name="page_identifier" id="page_identifier_hidden" value="{{ old('page_identifier') }}">
                        ...
                        <div class="mt-2 d-none" id="custom_page_identifier_container">
                            ...
                            <input type="text" class="form-control" id="custom_page_identifier" placeholder="e.g. europe-tours-landing" value="{{ old('page_identifier') && !array_key_exists(old('page_identifier'), $pages) ? old('page_identifier') : '' }}">
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('schema_type') is-invalid @enderror" id="schema_type_select" required>
                            ...
                            <option value="Organization" {{ old('schema_type') == 'Organization' ? 'selected' : '' }}>Organization Graph (Corporate Identity)</option>
                            <option value="WebSite" {{ old('schema_type') == 'WebSite' ? 'selected' : '' }}>WebSite Scope (Internal Search Bar)</option>
                            <option value="LocalBusiness" {{ old('schema_type') == 'LocalBusiness' ? 'selected' : '' }}>LocalBusiness (Contact Coordinates)</option>
                            <option value="TouristDestination" {{ old('schema_type') == 'TouristDestination' ? 'selected' : '' }}>TouristDestination (Travel Guides)</option>
                            <option value="FAQPage" {{ old('schema_type') == 'FAQPage' ? 'selected' : '' }}>FAQPage (Question Drops)</option>
                            <option value="BreadcrumbList" {{ old('schema_type') == 'BreadcrumbList' ? 'selected' : '' }}>BreadcrumbList (URL Pathing)</option>
                            <option value="FlightReservation" {{ old('schema_type') == 'FlightReservation' ? 'selected' : '' }}>FlightReservation (Booking Markups)</option>
                            <option value="custom" {{ old('schema_type') && !in_array(old('schema_type'), ['Organization','WebSite','LocalBusiness','TouristDestination','FAQPage','BreadcrumbList','FlightReservation']) ? 'selected' : '' }}>[Other... Type Custom Type]</option>
                        </select>
                        <input type="hidden" name="schema_type" id="schema_type_hidden" value="{{ old('schema_type') }}">
                        ...
                        <div class="mt-2 d-none" id="custom_schema_type_container">
                            ...
                            <input type="text" class="form-control" id="custom_schema_type" placeholder="e.g. TravelAgency" value="{{ old('schema_type') && !in_array(old('schema_type'), ['Organization','WebSite','LocalBusiness','TouristDestination','FAQPage','BreadcrumbList','FlightReservation']) ? old('schema_type') : '' }}">
                        </div>
                        @error('schema_type')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-12">
                        ...
                        <select class="form-select" id="status" name="status" required>
                            <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}>Active (Inject on destination page header)</option>
            ...
            <div class="mt-4 d-flex align-items-center gap-2">
                <button type="submit" class="btn btn-action rounded-pill px-5 py-2 fw-bold shadow-sm">
                    <i class="fa-solid fa-circle-check me-2"></i>Validate & Save Schema
                </button>
