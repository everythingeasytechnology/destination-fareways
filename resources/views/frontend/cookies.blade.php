...
<section class="bg-navy text-white py-5 mt-5">
    <div class="container py-4 text-center">
        <h1 class="display-5 fw-bold mb-3" data-aos="fade-up">{{ $page->title ?? 'Cookies Policy' }}</h1>
        <p class="lead text-muted-white mb-4" data-aos="fade-up" data-aos-delay="100">
            {{ $page->subtitle ?? 'How we use cookies and similar technologies on our website.' }}
        </p>
            ...
            <div class="col-lg-9" data-aos="fade-up" data-aos-delay="100">
                <div class="max-width-760 mx-auto">
                    ...
                    <div class="mt-5 border-top pt-4 text-muted small border-light text-center text-md-start">
                        Last modified: {{ date('M d, Y') }}. For privacy questions, please review our <a href="{{ route('privacy') }}" class="text-navy fw-semibold">Privacy Policy</a>.
                    </div>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (headings.length === 0) {
            tocList.innerHTML = '<li class="nav-item text-muted">Cookie Details</li>';
            return;
        }
